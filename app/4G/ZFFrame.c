...
#define ZF_SEND_FRAME_LEN    4


static INT8U ZF_SendRegister(void);		//发送注册帧
static INT8U ZF_SendHear(void);		//心跳
static INT8U ZF_SendDevState(void);		//充电桩状态
static INT8U   ZF_SendDevStateB(void);
_4G_SEND_TABLE ZFSendTable[ZF_SEND_FRAME_LEN] = {
	{0,    0,    SYS_DELAY_10s, 	ZF_SendRegister			},  //发送注册帧

	{0,    0,    SYS_DELAY_10s, 		ZF_SendHear				},	//心跳
	
	{0,    0,    SYS_DELAY_30s, 	ZF_SendDevState		},	//充电桩状态
	
	{0,    0,    SYS_DELAY_30s, 	ZF_SendDevStateB		},	//充电桩状态
};
...
*****************************************************************************/
static INT8U   ZF_SendRegister(void)
{
	INT8U sendbuf[20];
...
* Contributor  : 2021年3月19日
*****************************************************************************/
static INT8U   ZF_SendDevState(void)
{
	...
	{
		//充电输出电压		2字节
		data16 = PowerModuleInfo[GUN_A].OutputInfo.Vol;
		sendbuf[23] = (data16 >> 8) & 0x00ff;
		sendbuf[24] = data16 & 0x00ff;
		//充电输出电流		2字节
		data16 = PowerModuleInfo[GUN_A].OutputInfo.Cur*10;
		sendbuf[25] = (data16 >> 8) & 0x00ff;
		sendbuf[26] = data16 & 0x00ff;
		//输出继电器状态	1字节
		sendbuf[27] = 1;
		//已充电量			4字节
		data32 = puserinfo->TotalPower4 /1000;
		...
		//soc				2字节
		sendbuf[34] = 0x00;
		sendbuf[35] =  BMS_BCS_Context[GUN_A].SOC;
	}
...
static INT8U   ZF_SendDevStateB(void)
{
	...
	sendbuf[17] = (pdevnum[12] - '0') *0x10 +  (pdevnum[13] - '0');
	sendbuf[18] = (pdevnum[14] - '0') *0x10 +  (pdevnum[15] - '0');
	sendbuf[19] = 0x01;    //如果是单枪，此处为0，如果是双枪，1号枪是00，2号枪是01，以此类推，保证与档案里一致）
	//连接状态  	 	1字节
	...
	{
		//充电输出电压		2字节
		data16 = PowerModuleInfo[GUN_B].OutputInfo.Vol;
		sendbuf[23] = (data16 >> 8) & 0x00ff;
		sendbuf[24] = data16 & 0x00ff;
		//充电输出电流		2字节
		data16 = PowerModuleInfo[GUN_B].OutputInfo.Cur*10;
		sendbuf[25] = (data16 >> 8) & 0x00ff;
		sendbuf[26] = data16 & 0x00ff;
		//输出继电器状态	1字节
		sendbuf[27] = 1;
		...
		//soc				2字节
		sendbuf[34] = 0x00;
		sendbuf[35] =  BMS_BCS_Context[GUN_B].SOC;
	}
	ModuleSIM7600_SendData(LINK_ZF, sendbuf,36); //发送数据
	//OSTimeDly(SYS_DELAY_50ms); 
	return TRUE;
...
INT8U   ZF_SendStartCharge(void)
{
	...
	sendbuf[4] = 0x00;
	sendbuf[5] = 0x00;
	return ModuleSIM7600_SendData(LINK_ZF, sendbuf,6); //发送数据
	//OSTimeDly(SYS_DELAY_50ms); 
...
INT8U ZF_SendBill(void)
{
	...
	//电量1 	4字节   0.01
	sendbuf[41] = ((pmeter->CurPower4 -pmeter->StartPower4)/100)  & 0x000000ff;
	sendbuf[40] = (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 8) & 0x000000ff;
	sendbuf[39]= (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 16) & 0x000000ff;
	sendbuf[38] = (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 24)& 0x000000ff;
	//电量2 	4字节   0.01
	sendbuf[42] = 0;
	sendbuf[43] = 0;
	sendbuf[44] = 0;
	...
	sendbuf[53] = 0;
	//电量5 	4字节   0.01
	sendbuf[57] = ((pmeter->CurPower4 -pmeter->StartPower4)/100)  & 0x000000ff;
	sendbuf[56] = (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 8) & 0x000000ff;
	sendbuf[55]= (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 16) & 0x000000ff;
	sendbuf[54] = (((pmeter->CurPower4 -pmeter->StartPower4)/100) >> 24)& 0x000000ff;
