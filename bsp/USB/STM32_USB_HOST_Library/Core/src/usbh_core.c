...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
 ...
 //设备未连接或者不出于空闲状态
  if (((HCD_IsDeviceConnected(pdev) == 0)|| (HCD_IsPortEnabled(pdev) == 0))&& (phost->gState != HOST_IDLE)) 
  {
    ...
    {
      phost->gState = HOST_DEV_DISCONNECTED;        //将状态设置为未连接状态
    }
  }
    
  switch (phost->gState)    					//状态，逐个执行
  {
  
  case HOST_IDLE :								//空闲状态
    
    if (HCD_IsDeviceConnected(pdev))  			//设备连接上了
    {
      ...
      /* Apply a port RESET */
      HCD_ResetPort(pdev);						//发送端口复位指令
      
      /* User RESET callback*/
      phost->usr_cb->ResetDevice();				//回调函数，打印设备复位					
    }
    break;
    
  case HOST_WAIT_PRT_ENABLED:					
    if (pdev->host.PortEnabled == 1)			//端口是否使能，也就是设备是否连接
    { 
  ...
  case HOST_DEV_ATTACHED :
    ...
    /* Reset USB Device */				//复位USB
    if ( HCD_ResetPort(pdev) == 0)		//执行USB设备复位，若成功复位
    {
      phost->usr_cb->ResetDevice();		//打印设备复位
     
      /* Host is Now ready to start the Enumeration */  			//主机已经准备好枚举
      phost->device_prop.speed = HCD_GetCurrentSpeed(pdev);			//检测设备的速度
      
      phost->gState = HOST_ENUMERATION;
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);	//打印设备的速速  
      
      /* Open Control pipes */						//打开输入控制通道
  ...
  case HOST_USR_INPUT:    									//用户输入确认，这个函数返回用户的确认的信息
