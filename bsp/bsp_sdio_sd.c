...
//SD检测脚 
#define SD_DETECT_PORT                              GPIOG
#define SD_DETECT_PIN                               GPIO_Pin_3
#define SD_DETECT_PORT_CLK                          RCC_AHB1Periph_GPIOG    
...
void SD_LowLevel_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC | RCC_AHB1Periph_GPIOD | SD_DETECT_PORT_CLK, ENABLE);

	
		    //检测脚配置  PG3
	GPIO_InitStructure.GPIO_Pin = SD_DETECT_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;   
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;                //外部有上拉电阻
...
{
  static __IO uint8_t status = 0xff;
  __IO uint8_t curstatus;

  /*!< Check GPIO to detect SD */
  curstatus = (GPIO_ReadInputDataBit(SD_DETECT_PORT, SD_DETECT_PIN) == Bit_SET) ? SD_NOT_PRESENT : SD_PRESENT;
  if (curstatus != status)
  {
	  //消抖
    OSTimeDly(SYS_DELAY_5ms);
    if (curstatus == ( (GPIO_ReadInputDataBit(SD_DETECT_PORT, SD_DETECT_PIN) == Bit_SET) ? SD_NOT_PRESENT : SD_PRESENT) )
    {
        status = curstatus;
    }
  }
