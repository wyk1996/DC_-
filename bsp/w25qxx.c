...
#define SPI_CSEnable() SPI_FLASH_CS_LOW
#define SPI_CSDisable() SPI_FLASH_CS_HIGH
//4Kbytes为一个Sector
//16个扇区为1个Block
//W25Q256
//容量为32M字节,共有512个Block,8192个Sector 
INT8U SPI5_ReadWriteByte(INT8U data)
{
...
{ 								    
	...
	while(NumByteToRead--)
	{
		*pBuffer = SPI5_ReadWriteByte(0xff);
		pBuffer++;
	}
...
void FlishW25QXXInit(void)
{
//	INT16U FlashID;
//	INT8U writebuf[8] = {0x33,0x22,0x33,0x44,0x55,0x66,0x77,0x88};
//	INT8U readbuf[8];
	bsp_InitSPI2();   //SPI初始化
	W25QXX_Init();    //flash 初始化   
